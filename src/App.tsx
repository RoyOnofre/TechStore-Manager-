...
const SidebarItem: React.FC<SidebarItemProps> = ({ icon, label, active, onClick, roleColor = 'primary', badge }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-200 relative ${active
...
const App: React.FC = () => {
  const [currentScreen, setCurrentScreen] = useState<Screen>(() => {
    const savedScreen = localStorage.getItem('techstore_current_screen');
    const savedRole = localStorage.getItem('techstore_user_role');
    return savedRole ? (savedScreen as Screen || 'dashboard') : 'login';
  });

  const [isSidebarOpen, setIsSidebarOpen] = useState(true);
  const [selectedProductId, setSelectedProductId] = useState<string | null>(null);
  const [userRole, setUserRole] = useState<UserRole>(() => {
    return (localStorage.getItem('techstore_user_role') as UserRole) || 'admin';
  });
  ...
  const navigateTo = (screen: Screen, productId?: string) => {
    ...
    localStorage.setItem('techstore_current_screen', screen);
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        ...
        if (data && typeof data === 'object' && data.tienda_nombre) {
          setStoreName(String(data.tienda_nombre));
        }
      } catch (err) {
        console.error("Error cargando nombre de tienda:", err);
        setStoreName('TechStore');
      }
    };

    if (userRole) fetchConfig();
  ...
  const handleLogin = (role: UserRole, userData?: any) => {
    setUserRole(role);
    localStorage.setItem('techstore_user_role', role);
    localStorage.setItem('techstore_current_screen', 'dashboard');
    if (userData) {
      setCurrentUser({
  ...
  const handleLogout = () => {
    localStorage.removeItem('techstore_user_role');
    localStorage.removeItem('techstore_current_screen');
    localStorage.removeItem(`profile_${userRole}`);
    setUserRole('admin');
    navigateTo('login');
  };
  ...
  const getBgColor = () => {
    if (userRole === 'cajero') return 'bg-[#0a1a1a]';
    if (userRole === 'cliente') return 'bg-[#1a1022]';
  ...
  return (
    <div className={`min-h-screen flex ${getBgColor()} tech-pattern transition-colors duration-500 overflow-hidden`}>
      {showSidebar && isSidebarOpen && (
        <aside className="w-72 bg-surface-dark border-r border-white/5 flex flex-col z-50 animate-in slide-in-from-left duration-300">
          <div className="p-6 flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-primary flex items-center justify-center shadow-lg animate-pulse">
              <Package className="text-background-dark" size={24} />
            </div>
            <h1 className="text-xl font-bold tracking-tight text-white">{storeName}<span className={roleTextColor}>Pro</span></h1>
          </div>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
          ...
          <header className={`h-20 border-b ${roleBorderColor} bg-surface-dark/50 backdrop-blur-md flex items-center justify-between px-8 z-40`}>
            ...
            <div className="flex items-center gap-6">
              <div className="relative">
                ...
                {showNotifications && (
                  <div className="absolute right-0 mt-4 w-96 bg-surface-dark border border-primary/20 rounded-[32px] shadow-2xl overflow-hidden z-50 backdrop-blur-xl">
                      ...
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((n) => (
                          <div key={n.id} className={`p-5 border-b border-primary/5 hover:bg-primary/5 transition-colors flex gap-4 ${!n.read ? 'bg-primary/[0.02]' : ''}`}>
                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${n.type === 'warning' ? 'bg-amber-400/10 text-amber-400' :
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide">
          {renderScreen() || <DashboardScreen userRole={userRole} onNavigate={navigateTo} />}
        </div>
      </main>
    </div>
  );
